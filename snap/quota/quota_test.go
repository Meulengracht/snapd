...
func (ts *quotaTestSuite) TestNewGroup(c *C) {
	...
	tt := []struct {
		...
		{
			name:    "group1",
			limits:  resources.CreateQuotaResources(quantity.SizeMiB, 1, 100, []int{0}, 32),
			comment: "basic happy",
		},
		{
			name:    "biglimit",
			limits:  resources.CreateQuotaResources(quantity.Size(math.MaxUint64), 1, 100, []int{0}, 32),
			comment: "huge limit happy",
		},
		{
			name:    "zero",
			limits:  resources.CreateQuotaResources(0, 0, 0, nil, 0),
...
func (ts *quotaTestSuite) TestSimpleSubGroupVerification(c *C) {
	tt := []struct {
		...
		{
			rootlimits: resources.CreateQuotaResources(quantity.SizeMiB, 1, 100, []int{0}, 32),
			subname:    "sub",
			sublimits:  resources.CreateQuotaResources(quantity.SizeMiB, 1, 100, []int{0}, 32),
			comment:    "basic sub group with same quota as parent happy",
		},
		{
			rootlimits: resources.CreateQuotaResources(quantity.SizeMiB, 1, 100, []int{0}, 32),
			subname:    "sub",
			sublimits:  resources.CreateQuotaResources(quantity.SizeMiB/2, 1, 50, []int{0}, 16),
		...
		},
		{
			rootlimits: resources.CreateQuotaResources(quantity.SizeMiB, 1, 100, []int{0}, 32),
			subname:    "sub",
			sublimits:  resources.CreateQuotaResources(quantity.SizeMiB*2, 0, 0, nil, 0),
		...
		},
		{
			rootlimits: resources.CreateQuotaResources(quantity.SizeMiB, 1, 100, []int{0}, 32),
			subname:    "sub",
			sublimits:  resources.CreateQuotaResources(quantity.SizeMiB/2, 2, 100, nil, 0),
		...
		},
		{
			rootlimits: resources.CreateQuotaResources(quantity.SizeMiB, 1, 100, []int{0}, 32),
			subname:    "sub",
			sublimits:  resources.CreateQuotaResources(quantity.SizeMiB/2, 1, 50, []int{1}, 0),
		...
		},
		{
			rootlimits: resources.CreateQuotaResources(quantity.SizeMiB, 1, 100, []int{0}, 32),
			subname:    "sub",
			sublimits:  resources.CreateQuotaResources(quantity.SizeMiB/2, 1, 100, []int{0}, 64),
		...
		},
		{
			rootlimits: resources.CreateQuotaResources(quantity.SizeMiB, 1, 100, []int{0}, 32),
			subname:    "zero",
			sublimits:  resources.CreateQuotaResources(0, 0, 0, nil, 0),
			err:        `quota group must have at least one resource quota set`,
...
func (ts *quotaTestSuite) TestComplexSubGroups(c *C) {
	rootGrp, err := quota.NewGroup("myroot", resources.CreateQuotaResources(quantity.SizeMiB, 0, 0, nil, 0))
	c.Assert(err, IsNil)

	// try adding 2 sub-groups with total quota split exactly equally
	sub1, err := rootGrp.NewSubGroup("sub1", resources.CreateQuotaResources(quantity.SizeMiB/2, 0, 0, nil, 0))
	c.Assert(err, IsNil)
	c.Assert(sub1.SliceFileName(), Equals, "snap.myroot-sub1.slice")

	sub2, err := rootGrp.NewSubGroup("sub2", resources.CreateQuotaResources(quantity.SizeMiB/2, 0, 0, nil, 0))
	c.Assert(err, IsNil)
	c.Assert(sub2.SliceFileName(), Equals, "snap.myroot-sub2.slice")

	// adding another sub-group to this group fails
	_, err = rootGrp.NewSubGroup("sub3", resources.CreateQuotaResources(1, 0, 0, nil, 0))
	...
	subsub1, err := sub1.NewSubGroup("subsub1", resources.CreateQuotaResources(quantity.SizeMiB/2, 0, 0, nil, 0))
	c.Assert(err, IsNil)
	...
	subsubsub1, err := subsub1.NewSubGroup("subsubsub1", resources.CreateQuotaResources(quantity.SizeMiB/4, 0, 0, nil, 0))
	c.Assert(err, IsNil)
...
func (ts *quotaTestSuite) TestAddAllNecessaryGroupsAvoidsInfiniteRecursion(c *C) {
	grp, err := quota.NewGroup("infinite-group", resources.CreateQuotaResources(quantity.SizeGiB, 0, 0, nil, 0))
	c.Assert(err, IsNil)

	grp2, err := grp.NewSubGroup("infinite-group2", resources.CreateQuotaResources(quantity.SizeGiB, 0, 0, nil, 0))
	c.Assert(err, IsNil)
	...
	grp3, err := grp2.NewSubGroup("infinite-group3", resources.CreateQuotaResources(quantity.SizeGiB, 0, 0, nil, 0))
	c.Assert(err, IsNil)
...
func (ts *quotaTestSuite) TestAddAllNecessaryGroups(c *C) {
	...
	grp1, err := quota.NewGroup("myroot", resources.CreateQuotaResources(quantity.SizeGiB, 0, 0, nil, 0))
	c.Assert(err, IsNil)
	...
	grp2, err := quota.NewGroup("myroot2", resources.CreateQuotaResources(quantity.SizeGiB, 0, 0, nil, 0))
	c.Assert(err, IsNil)
	...
	subgrp1, err := grp1.NewSubGroup("mysub1", resources.CreateQuotaResources(quantity.SizeGiB, 0, 0, nil, 0))
	c.Assert(err, IsNil)
	// add grp2 as well
	err = qs.AddAllNecessaryGroups(grp2)
	c.Assert(err, IsNil)
	...
	grp3, err := quota.NewGroup("myroot3", resources.CreateQuotaResources(quantity.SizeGiB, 0, 0, nil, 0))
	c.Assert(err, IsNil)

	subgrp3, err := grp3.NewSubGroup("mysub3", resources.CreateQuotaResources(quantity.SizeGiB, 0, 0, nil, 0))
	c.Assert(err, IsNil)

	subsubgrp3, err := subgrp3.NewSubGroup("mysubsub3", resources.CreateQuotaResources(quantity.SizeGiB, 0, 0, nil, 0))
	c.Assert(err, IsNil)
	...
	// branches
	grp4, err := quota.NewGroup("myroot4", resources.CreateQuotaResources(quantity.SizeGiB, 0, 0, nil, 0))
	c.Assert(err, IsNil)

	subgrp4, err := grp4.NewSubGroup("mysub4", resources.CreateQuotaResources(quantity.SizeGiB/2, 0, 0, nil, 0))
	c.Assert(err, IsNil)

	subgrp5, err := grp4.NewSubGroup("mysub5", resources.CreateQuotaResources(quantity.SizeGiB/2, 0, 0, nil, 0))
	c.Assert(err, IsNil)
...
func (ts *quotaTestSuite) TestResolveCrossReferencesLimitCheckSkipsSelf(c *C) {
	...
	subgrp2, err := subgrp1.NewSubGroup("mysub2", resources.CreateQuotaResources(quantity.SizeGiB, 0, 0, nil, 0))
	c.Assert(err, IsNil)
...
func (ts *quotaTestSuite) TestCurrentMemoryUsage(c *C) {
	...
	grp1, err := quota.NewGroup("group", resources.CreateQuotaResources(quantity.SizeGiB, 0, 0, nil, 0))
	c.Assert(err, IsNil)
