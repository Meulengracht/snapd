...
func mockQuotas(st *state.State, c *check.C) {
	err := servicestatetest.MockQuotaInState(st, "foo", "", nil, resources.CreateQuotaResources(11000, 0, 0, nil, 0))
	c.Assert(err, check.IsNil)
	err = servicestatetest.MockQuotaInState(st, "bar", "foo", nil, resources.CreateQuotaResources(6000, 0, 0, nil, 0))
	c.Assert(err, check.IsNil)
	err = servicestatetest.MockQuotaInState(st, "baz", "foo", nil, resources.CreateQuotaResources(5000, 0, 0, nil, 0))
	c.Assert(err, check.IsNil)
}
...
func (s *apiQuotaSuite) TestPostEnsureQuotaUnhappy(c *check.C) {
	r := daemon.MockServicestateCreateQuota(func(st *state.State, name string, parentName string, snaps []string, resourceLimits resources.QuotaResources) (*state.TaskSet, error) {
		...
		c.Check(resourceLimits, check.DeepEquals, resources.CreateQuotaResources(quantity.Size(1000), 0, 0, nil, 0))
...
func (s *apiQuotaSuite) TestPostEnsureQuotaUpdateHappy(c *check.C) {
	st := s.d.Overlord().State()
	st.Lock()
	err := servicestatetest.MockQuotaInState(st, "ginger-ale", "", nil, resources.CreateQuotaResources(5000, 0, 0, nil, 0))
	st.Unlock()
	c.Assert(err, check.IsNil)
...
func (s *apiQuotaSuite) TestPostRemoveQuotaConflict(c *check.C) {
	st := s.d.Overlord().State()
	st.Lock()
	err := servicestatetest.MockQuotaInState(st, "ginger-ale", "", []string{"some-snap"}, resources.CreateQuotaResources(5000, 0, 0, nil, 0))
	st.Unlock()
	c.Assert(err, check.IsNil)
...
func (s *apiQuotaSuite) TestListQuotas(c *check.C) {
	...
	r := daemon.MockGetQuotaUsage(func(grp *quota.Group) (*client.QuotaValues, error) {
		...
		switch grp.Name {
		case "bar":
			return &client.QuotaValues{Memory: quantity.Size(500)}, nil
		case "baz":
			return &client.QuotaValues{Memory: quantity.Size(1000)}, nil
		case "foo":
			return &client.QuotaValues{Memory: quantity.Size(5000)}, nil
		default:
