...
func (ts *quotaTestSuite) TestNewGroup(c *C) {
	...
	tt := []struct {
		...
		{
			name:    "group1",
			limits:  resources.CreateQuotaResources(quantity.SizeMiB, 0, 0, nil, 0),
			comment: "basic happy",
		},
		{
			name:    "biglimit",
			limits:  resources.CreateQuotaResources(quantity.Size(math.MaxUint64), 0, 0, nil, 0),
...
func (ts *quotaTestSuite) TestSimpleSubGroupVerification(c *C) {
	tt := []struct {
		...
		{
			rootlimits: resources.CreateQuotaResources(quantity.SizeMiB, 0, 0, nil, 0),
			subname:    "sub",
			sublimits:  resources.CreateQuotaResources(quantity.SizeMiB, 0, 0, nil, 0),
