...
func mockQuotas(st *state.State, c *check.C) {
	...
	c.Assert(err, check.IsNil)
	err = servicestatetest.MockQuotaInState(st, "baz", "foo", nil, resources.CreateQuotaResources(5000, 0, 100, nil, 0))
	c.Assert(err, check.IsNil)
}
...
func (s *apiQuotaSuite) TestPostEnsureQuotaUnhappy(c *check.C) {
	r := daemon.MockServicestateCreateQuota(func(st *state.State, name string, parentName string, snaps []string, resourceLimits resources.QuotaResources) (*state.TaskSet, error) {
		...
		c.Check(resourceLimits, check.DeepEquals, resources.CreateQuotaResources(quantity.Size(1000), 1, 100, nil, 128))
...
func (s *apiQuotaSuite) TestPostEnsureQuotaCreateHappy(c *check.C) {
	...
	r := daemon.MockServicestateCreateQuota(func(st *state.State, name string, parentName string, snaps []string, resourceLimits resources.QuotaResources) (*state.TaskSet, error) {
		...
		c.Check(resourceLimits, check.DeepEquals, resources.CreateQuotaResources(quantity.Size(1000), 1, 100, nil, 256))
...
func (s *apiQuotaSuite) TestPostEnsureQuotaUpdateCpu2Happy(c *check.C) {
	...
	r = daemon.MockServicestateUpdateQuota(func(st *state.State, name string, opts servicestate.QuotaGroupUpdate) (*state.TaskSet, error) {
		...
		c.Assert(opts, check.DeepEquals, servicestate.QuotaGroupUpdate{
			AddSnaps:          []string{"some-snap"},
			NewResourceLimits: resources.CreateQuotaResources(0, 1, 100, []int{0, 1}, 0),
		})
		ts := state.NewTaskSet(st.NewTask("foo-quota", "..."))
		return ts, nil
	})
...
func (s *apiQuotaSuite) TestPostRemoveQuotaConflict(c *check.C) {
	st := s.d.Overlord().State()
	st.Lock()
	err := servicestatetest.MockQuotaInState(st, "ginger-ale", "", []string{"some-snap"}, resources.CreateQuotaResources(5000, 1, 50, nil, 32))
	st.Unlock()
	c.Assert(err, check.IsNil)
