...
func (s *servicestateQuotasSuite) TestCreateQuotaInState(c *C) {
	...
	grp1, newGrps, err := internal.CreateQuotaInState(st, "foogroup", nil, nil, resources.CreateQuotaResources(quantity.SizeGiB, 0, 0, nil, 0), nil)
	c.Assert(err, IsNil)
	...
	grp3, newGrps, err := internal.CreateQuotaInState(st, "group-2", grp1, []string{"snap1", "snap2"}, resources.CreateQuotaResources(quantity.SizeGiB, 0, 0, nil, 0), nil)
	c.Assert(err, IsNil)
